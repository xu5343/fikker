...
service iptables stop && chkconfig iptables off && service httpd stop && service nginx stop && chkconfig httpd off && chkconfig nginx off && systemctl stop firewalld.service && systemctl disable firewalld.service && systemctl stop httpd.service && systemctl stop nginx.service && systemctl disable httpd.service && systemctl disable nginx.service && yum -y install wget && cd $FikkerInstallDir && wget -c --no-check-certificate https://raw.githubusercontent.com/xu5343/fikker/main/$FikkerNewVersion.tar.gz && tar zxf $FikkerNewVersion.tar.gz && rm -rf $FikkerNewVersion.tar.gz && cd $FikkerNewVersion && ./fikkerd.sh install && ./fikkerd.sh start && cd $FikkerInstallDir && sleep 5 && echo 'finished!'
